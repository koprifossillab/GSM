...
- **배경지도가 없다.** OpenStreetMap 을 깔았다가 기관 망 IP 가 막혀
  `403 Access blocked` 가 화면을 덮었다(003). 지금 기본값은 "없음" 이고,
  VWorld 열쇠를 받으면 고르개에 오른다. **VWorld 는 코드만 적었을 뿐
  한 번도 그려 보지 못했다** — 열쇠가 없어서다
...
| `vworld_key` | 배경지도 열쇠. 있으면 고르개에 VWorld 가 오른다 |
