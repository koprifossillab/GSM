...
**화면에 약자를 풀어 쓰지 않는다** — `GSM (GreatStoneMap)` 같은 표기를 두지
않는다. 풀이가 필요한 곳은 이 문단 하나다.
...
- **제품이 도는 길은 언제나 `/openapi/wms` + 인증키다.** 식별되는 쪽으로
  나가는 것이 옳고, 문서에 없는 주소는 예고 없이 닫혀도 할 말이 없다
- `GSM_DEV_DIRECT_WMS=1` 은 **개발 스위치**다. 켜면 상류 요청이 GeoServer 로
  곧장 간다. 인증키 심사를 기다리는 동안 뷰어를 끝까지 굴려보려고 둔 것이고,
  기본값은 꺼짐이다. **운영에서 켜지 않는다**
